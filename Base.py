...
def check_numberMinMax( num_min:float = 0.0,num_max:float = 100, text_in:str = 'Введите положительное число', text_breac:str = 'Вы ввели число не правильно, введите его заново:'): 
    '''
    Проверка, является ли введённые данные числом. И это число больше num
    Если нет то повторяется запрос заново.
    text_in - техт запроса в консоли
    ...
    while number.isdigit() == False or number_in.count('-')>1 or (number_in.count('-')==1 and number_in[0]!='-') or number_in.count('.')>1 or float(number_in)<num_min or float(number_in)>num_max:
        number_in = input(text_breac)
        number = number_in.replace('-','0')
        number = number.replace('.','0')
